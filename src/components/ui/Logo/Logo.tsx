...
/**
 * PV Link Energy logo mark (public/LogoIcon.svg, inlined so it scales
 * crisply and needs no extra request). Multi-colour brand cube — left
 * as-is on the monochrome UI.
 */
export default function Logo({
  title = "PV Link Energy",
  className = "",
}: LogoProps) {
  return (
    <span className={`logo ${className}`.trim()} role="img" aria-label={title}>
      <svg
